...
class yoloRotate:
    def __init__(self, filename,image_ext, label_ext, angle):
        """
        1. Checking the image paths
        2. Initializing the Imageslabel_ext
        3. Defining the transformation Matrix
        """
        assert os.path.isfile(filename + image_ext)
        assert os.path.isfile(os.path.join(filename + label_ext))
    ...
    def rotateYoloBB(self):
        myFile = open(self.filename + self.label_ext,"tr")
        f1 = []
        ...
        for bbox in f1:
            points = []
            (startX, startY, endX, endY) = self.yoloFormattocv(float(bbox[1]), float(bbox[2]),
                                                                               float(bbox[3]), float(bbox[4]), H, W)
            points.append((startX, startY))
            points.append((endX, startY))
            points.append((startX, endY))
            ...
            maxX = np.max(xPoints)
            maxY = np.max(yPoints)
            new_bbox.append([bbox[0], minX, minY, maxX, maxY])
...
if __name__=="__main__":   
    ...
    for e in tqdm(ext):
        ...
        for filename in files:
            ...
            angles = [0, 30, 45, 60, 90, 120, 135, 150, 180, 210, 240, 270]
            for angle in angles:
                im = yoloRotate(image_name,image_ext,".txt", angle)
                ...
                #saving the Bbox coordinates in YOLO format in same folder as its augmented image data
                for bb in bbox:
